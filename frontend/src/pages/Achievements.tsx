...
export default function Achievements() {
  ...
  const rarityBorder = (r: string) => r==='legendary'?'hsl(var(--primary))':r==='epic'?'hsl(280 60% 45%)':r==='rare'?'hsl(200 70% 40%)':'hsl(var(--border))';
      ...
      <div style={{ display:'flex', alignItems:'flex-start', gap:'0.75rem' }}>
        ...
        <div style={{ flex:1 }}>
          ...
          <div style={{ display:'flex', justifyContent:'space-between' }}>
            <span style={{ fontSize:'0.75rem', fontWeight:'bold', fontFamily:'var(--font-mono)', color:'hsl(var(--xp))' }}>+{a.xpReward} XP</span>
  ...
  return (
    <div className="page fade-in">
      ...
      <div className="rpg-card" style={{ padding:'1.25rem', marginBottom:'1.5rem', borderColor:'hsl(var(--gold-line))' }}>
        ...
        <div className="xp-bar" style={{ height:'0.75rem' }}>
          <div className="xp-bar-fill" style={{ width:achievements.length>0?`${(unlocked.length/achievements.length)*100}%`:'0%', background:'linear-gradient(90deg, hsl(var(--gold-line)), hsl(var(--primary)))' }} />
        </div>
      </div>
      {unlocked.length > 0 && (
