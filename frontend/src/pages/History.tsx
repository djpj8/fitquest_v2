  ...
  return (
    <div style={{ padding:'1.5rem' }} className="fade-in">
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'1rem', marginBottom:'1.5rem' }}>
        {[{label:'Quest Totali',value:(logs as any[]).length,icon:'⚔️',color:'hsl(43 85% 55%)'},{label:'XP Guadagnati',value:totalXp.toLocaleString(),icon:'⚡',color:'hsl(160 60% 50%)'},{label:'Tempo Totale',value:formatDuration(totalTime),icon:'⏱️',color:'hsl(280 60% 60%)'}].map(s => (
          ...
          {(logs as any[]).map((log:any) => (
            <div key={log.id} className="rpg-card" style={{ overflow:'hidden' }}>
              <button onClick={() => setExpanded(expanded===log.id?null:log.id)} style={{ width:'100%', display:'flex', alignItems:'center', gap:'1rem', padding:'1rem', background:'none', border:'none', cursor:'pointer', textAlign:'left' }}>
                <div style={{ width:'3rem', height:'3rem', borderRadius:'0.5rem', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'1.5rem', background:'hsl(43 85% 20% / 0.4)', border:'1px solid hsl(43 85% 35%)', flexShrink:0 }}>⚔️</div>
                ...
                <div style={{ textAlign:'right', flexShrink:0 }}>
                  <div style={{ fontWeight:'bold', fontFamily:'var(--font-mono)', color:'hsl(160 60% 50%)' }}>+{log.xpEarned} XP</div>
