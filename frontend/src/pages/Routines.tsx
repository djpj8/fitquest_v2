    ...
    <div style={{ padding: '1.5rem' }} className="fade-in">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <button onClick={() => saveMutation.mutate()} disabled={!form.name || saveMutation.isPending} style={{ padding: '0.75rem', borderRadius: '0.5rem', fontWeight: 'bold', fontFamily: 'var(--font-serif)', border: 'none', cursor: form.name ? 'pointer' : 'not-allowed', background: !form.name ? 'hsl(var(--muted))' : 'linear-gradient(135deg, hsl(43 85% 45%), hsl(43 85% 60%))', color: !form.name ? 'hsl(var(--muted-foreground))' : 'hsl(220 20% 6%)' }}>
        ...
        <div className="rpg-card" style={{ padding: '1rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem', maxHeight: '30rem', overflowY: 'auto' }}>
            {filtered.map(ex => { const sel = !!form.exercises.find(e => e.exerciseId === ex.id); return (
              <div key={ex.id} onClick={() => !sel && addEx(ex)} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem', borderRadius: '0.5rem', cursor: sel ? 'default' : 'pointer', background: sel ? 'hsl(43 85% 20% / 0.4)' : 'hsl(var(--muted))', border: `2px solid ${sel ? 'hsl(43 85% 55%)' : 'transparent'}` }}>
                ...
                {sel && <span style={{ fontSize: '0.75rem', color: 'hsl(43 95% 65%)' }}>✓</span>}
              </div>
            ); })}
          </div>
        </div>
  ...
  return (
    <div style={{ padding: '1.5rem' }} className="fade-in">
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
        ...
        <button onClick={() => setCreating(true)} style={{ padding: '0.5rem 1rem', borderRadius: '0.5rem', fontWeight: 'bold', fontFamily: 'var(--font-serif)', border: 'none', cursor: 'pointer', background: 'linear-gradient(135deg, hsl(43 85% 45%), hsl(43 85% 60%))', color: 'hsl(220 20% 6%)' }}>⚒️ Nuova</button>
      </div>
          ...
          {(routines as any[]).map(r => (
            <div key={r.id} className="rpg-card" style={{ padding: '1rem' }}>
              ...
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                <button onClick={() => onNavigate && onNavigate('log-workout', r.id)} style={{ flex: 1, padding: '0.375rem', borderRadius: '0.5rem', fontSize: '0.8rem', fontWeight: 'bold', fontFamily: 'var(--font-serif)', border: 'none', cursor: 'pointer', background: 'linear-gradient(135deg, hsl(43 85% 45%), hsl(43 85% 60%))', color: 'hsl(220 20% 6%)' }}>⚔️ Inizia</button>
