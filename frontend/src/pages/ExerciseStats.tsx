            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
              {exercises.map(ex => (
                <button key={ex} onClick={() => setSelected(ex)}
                  style={{ padding: '0.375rem 0.75rem', borderRadius: '0.5rem', fontSize: '0.8rem', cursor: 'pointer', border: `1px solid ${currentEx === ex ? 'hsl(var(--primary))' : 'hsl(var(--border))'}`, background: currentEx === ex ? 'hsl(43 85% 20% / 0.4)' : 'hsl(var(--muted))', color: currentEx === ex ? 'hsl(var(--primary))' : 'hsl(var(--foreground))' }}>
                  {ex}
                </button>
              ))}
            </div>
          ...
          {currentEx && (
            <>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem', marginBottom: '1.5rem' }}>
                {[
                  { icon: '🏆', label: 'Massimo storico', value: `${maxWeight} kg`, color: 'hsl(43 85% 55%)' },
                  { icon: '📅', label: 'Sessioni registrate', value: data.length, color: 'hsl(200 70% 55%)' },
                  { icon: '📈', label: 'Progressione', value: progress !== null ? `${Number(progress) > 0 ? '+' : ''}${progress}%` : '—', color: Number(progress) > 0 ? 'hsl(160 60% 50%)' : 'hsl(0 70% 55%)' },
                ].map(s => (
              ...
              <div className="rpg-card" style={{ padding: '1.25rem' }}>
                ...
                <ResponsiveContainer width="100%" height={250}>
                  <LineChart data={chartData}>
                    ...
                    <Line type="monotone" dataKey="peso" stroke="hsl(43 85% 55%)" strokeWidth={2} dot={{ fill: 'hsl(43 85% 55%)', r: 5 }} activeDot={{ r: 7 }} />
                  </LineChart>
                </ResponsiveContainer>
                ...
                <div style={{ marginTop: '1rem', display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                  {[...data].map((d, i) => (
                    <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '0.5rem 0.75rem', borderRadius: '0.375rem', background: i === 0 ? 'hsl(43 85% 20% / 0.3)' : 'transparent', fontSize: '0.875rem' }}>
                      <span style={{ color: 'hsl(var(--muted-foreground))' }}>{new Date(d.date).toLocaleDateString('it-IT')}</span>
                      <span style={{ fontFamily: 'var(--font-mono)', color: i === 0 ? 'hsl(43 95% 65%)' : 'hsl(var(--foreground))' }}>{d.maxWeight} kg {i === 0 && '🏆'}</span>
                    </div>
                  ))}
                </div>
              </div>
