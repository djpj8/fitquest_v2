...
export default function LogWorkout({ onNavigate, initialRoutineId }: { onNavigate: (p: string, id?: number) => void; initialRoutineId?: number }) {
  ...
  if (result) return (
    <div style={{ padding: '1.5rem', maxWidth: '32rem', margin: '0 auto' }} className="fade-in">
      <div className="rpg-card" style={{ padding: '2rem', textAlign: 'center', borderColor: 'hsl(var(--gold-line))', boxShadow: '0 0 40px hsl(var(--gold-tint-strong))' }}>
        ...
        <div style={{ fontSize: '3rem', fontWeight: 'bold', fontFamily: 'var(--font-serif)', color: 'hsl(var(--xp))', marginBottom: '1rem' }}>+{result.xpEarned} XP</div>
        {result.newAchievements?.length > 0 && (
          <div style={{ marginBottom: '1rem', padding: '1rem', borderRadius: '0.5rem', background: 'hsl(var(--gold-tint-strong))', border: '1px solid hsl(var(--gold-line))' }}>
        ...
        <button onClick={() => onNavigate('dashboard')} style={{ width: '100%', padding: '0.75rem', borderRadius: '0.5rem', fontWeight: 'bold', fontFamily: 'var(--font-serif)', border: 'none', cursor: 'pointer', background: 'var(--btn-gradient)', color: 'hsl(var(--btn-text))' }}>
          Torna alla Base
        </button>
      </div>
    </div>
    ...
    <div className="page fade-in">
      ...
      <div className="grid-split">
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            <div className="rpg-card" style={{ padding: '1rem' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                {(routines as any[]).map((r: any) => (
                  <button key={r.id} onClick={() => loadRoutine(r)} style={{ padding: '0.375rem 0.75rem', borderRadius: '0.5rem', fontSize: '0.8rem', cursor: 'pointer', border: `1px solid ${workoutName === r.name ? 'hsl(var(--primary))' : 'hsl(var(--border))'}`, background: workoutName === r.name ? 'hsl(var(--gold-tint-strong))' : 'hsl(var(--muted))', color: workoutName === r.name ? 'hsl(var(--primary))' : 'hsl(var(--foreground))' }}>📋 {r.name}</button>
                ))}
              </div>
            </div>
          )}
            ...
            <div key={we.exercise.id} className="rpg-card" style={{ padding: '1rem' }}>
              ...
              {we.sets.map((set, si) => (
                <div key={si} style={{ display: 'grid', gridTemplateColumns: '2rem 1fr 1fr 2rem', gap: '0.5rem', alignItems: 'center', marginBottom: '0.25rem' }}>
                  ...
                  <button onClick={() => updateSet(ei, si, 'completed', !set.completed)} style={{ width: '2rem', height: '2rem', borderRadius: '50%', border: `1px solid ${set.completed ? 'hsl(160 60% 40%)' : 'hsl(var(--border))'}`, background: set.completed ? 'hsl(var(--xp))' : 'hsl(var(--muted))', cursor: 'pointer', fontSize: '0.875rem' }}>{set.completed ? '✓' : ''}</button>
                </div>
              ))}
          ...
          <button onClick={() => logMutation.mutate()} disabled={(workoutExercises as WorkoutExercise[]).length === 0 || logMutation.isPending} style={{ padding: '0.75rem', borderRadius: '0.5rem', fontWeight: 'bold', fontFamily: 'var(--font-serif)', border: 'none', cursor: (workoutExercises as WorkoutExercise[]).length === 0 ? 'not-allowed' : 'pointer', background: (workoutExercises as WorkoutExercise[]).length === 0 ? 'hsl(var(--muted))' : 'var(--btn-gradient)', color: (workoutExercises as WorkoutExercise[]).length === 0 ? 'hsl(var(--muted-foreground))' : 'hsl(var(--btn-text))' }}>
        ...
        <div className="rpg-card" style={{ padding: '1rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.75rem' }}>
            ...
            <button onClick={() => setShowAddForm(!showAddForm)} style={{ padding: '0.375rem 0.75rem', borderRadius: '0.5rem', fontSize: '0.75rem', fontWeight: 'bold', fontFamily: 'var(--font-serif)', cursor: 'pointer', border: 'none', background: showAddForm ? 'hsl(var(--muted))' : 'var(--btn-gradient)', color: showAddForm ? 'hsl(var(--muted-foreground))' : 'hsl(var(--btn-text))' }}>
              {showAddForm ? '✕' : '➕ Custom'}
            </button>
          </div>
          {showAddForm && (
            <div style={{ marginBottom: '1rem', padding: '1rem', borderRadius: '0.5rem', background: 'hsl(var(--muted))', border: '1px solid hsl(var(--border))', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              ...
              <button onClick={() => addExMutation.mutate()} disabled={!newEx.name || addExMutation.isPending} style={{ padding: '0.5rem', borderRadius: '0.5rem', fontWeight: 'bold', border: 'none', cursor: newEx.name ? 'pointer' : 'not-allowed', background: !newEx.name ? 'hsl(var(--muted))' : 'var(--btn-gradient)', color: !newEx.name ? 'hsl(var(--muted-foreground))' : 'hsl(var(--btn-text))' }}>
          ...
          <div style={{ display: 'flex', gap: '0.25rem', flexWrap: 'wrap', marginBottom: '0.75rem' }}>
            {CATEGORIES.map(cat => (
              <button key={cat} onClick={() => setFilterCat(cat)} style={{ padding: '0.2rem 0.5rem', borderRadius: '999px', fontSize: '0.65rem', cursor: 'pointer', border: `1px solid ${filterCat === cat ? 'hsl(var(--gold-line))' : 'transparent'}`, background: filterCat === cat ? 'hsl(var(--gold-tint-strong))' : 'transparent', color: filterCat === cat ? 'hsl(var(--primary))' : 'hsl(var(--muted-foreground))', textTransform: 'capitalize' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem', maxHeight: '26rem', overflowY: 'auto' }}>
            {filtered.map(ex => {
              ...
              return (
                <div key={ex.id} onClick={() => !sel && addExercise(ex)} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 0.75rem', borderRadius: '0.5rem', cursor: sel ? 'default' : 'pointer', background: sel ? 'hsl(var(--gold-tint-strong))' : 'hsl(var(--muted))', border: `2px solid ${sel ? 'hsl(var(--primary))' : 'transparent'}` }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', flexWrap: 'wrap' }}>
                      ...
                      {ex.type === 'calisthenics' && <span style={{ fontSize: '0.6rem', padding: '0.1rem 0.3rem', borderRadius: '999px', background: 'hsl(160 60% 20%)', color: 'hsl(var(--xp))', flexShrink: 0 }}>🤸</span>}
                      {ex.userId && <span style={{ fontSize: '0.6rem', padding: '0.1rem 0.3rem', borderRadius: '999px', background: 'hsl(280 60% 20%)', color: 'hsl(var(--violet))', flexShrink: 0 }}>✏️</span>}
                      {sel && <span style={{ fontSize: '0.6rem', padding: '0.1rem 0.3rem', borderRadius: '999px', background: 'hsl(43 85% 30%)', color: 'hsl(var(--primary))', flexShrink: 0 }}>✓</span>}
                    </div>
                  ...
                  <div style={{ display: 'flex', gap: '0.25rem', flexShrink: 0 }} onClick={e => e.stopPropagation()}>
                    <button onClick={() => favMutation.mutate(ex.id)} style={{ width: '1.75rem', height: '1.75rem', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.875rem', cursor: 'pointer', border: 'none', background: isFav ? 'hsl(var(--gold-tint-strong))' : 'transparent', color: isFav ? 'hsl(var(--primary))' : 'hsl(var(--muted-foreground))' }}>{isFav ? '⭐' : '☆'}</button>
